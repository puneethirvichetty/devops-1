echo 'export harborip=`curl -H "Metadata-Flavor: Google" http://metadata/computeMetadata/v1/instance/network-interfaces/0/access-configs/0/external-ip`' >> ~/.bash_profile
source ~/.bash_profile
...
cd /
cd /etc/docker/
sudo sed -i 's/target/'$harborip'/' daemon.json
cd /
